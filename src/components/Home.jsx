...
const mapDispatchToProps = (dispatch) => ({
  ...
  getArtist: () => dispatch(getArtistAction()),
});

const Home = (props) => {
  const getArtistWithTrackList = () => {
    props.setLoading(true);
    props.setId(document.getElementById("id").value);
    props.getArtist();
  };
  return (
    <>
      {/* input an artist ID and click the button.
	it sets the id as an artist's ID, and then fetch with this id.
	after receiving data from a fetch, it reads the state and fires another fetch for a tracklist */}
      <h1>Home</h1>
      <input id="id" placeholder="artist id" />
      <Button onClick={() => getArtistWithTrackList()}>Click me</Button>
    </>
  );
};
